...
------------------------------------------------------------------------------------------------------
----Login Stored procedure for verified User------
...
BEGIN
declare @x int;
IF EXISTS(Select * from User_authentication where Email=@email AND Password=@password)
 Begin
--- check it's verified or not---
   set @x= (select IsVerified from TBL_Users where Email=@email)
    IF @x=1
        begin
		Print'Login'
        end
...
User_Login 'amit12@gmail.com','amit@123'

User_Login 'singh12@gmail.com','sonal@123'
